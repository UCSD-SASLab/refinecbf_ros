...
class NominalControlHJNP:
    ...
    def get_nominal_control_table(self,time_index):
        ...
        for i in tqdm(range(self.grid_shape[0])):
            if self.grid.ndim == 1:
                table[i] = self.get_nominal_control(self.grid_states_np[i],time_index)
            else:
                for j in range(self.grid_shape[1]):
                    if self.grid.ndim == 2:
                        table[i, j] = self.get_nominal_control(self.grid_states_np[i, j],time_index)
                    else:
                        for k in range(self.grid_shape[2]):
                            if self.grid.ndim == 3:
                                table[i, j, k] = self.get_nominal_control(
                                    self.grid_states_np[i, j, k],time_index
                                )
        # return hj.utils.multivmap(self.get_nominal_control, jnp.arange(self.grid.ndim))(
        #     self.grid.states
        # )
